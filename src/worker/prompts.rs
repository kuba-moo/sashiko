...
const STAGE8_OUTPUT_INSTRUCTION: &str = r#"Return ONLY a compact JSON deduplication plan with this shape:
{
  "concerns": {
    "keep": ["finding-id"],
    "merge": [
      {
        "ids": ["finding-id", "matching-finding-id"],
        "base": "finding-id",
        "replace": {
          "reasoning": "optional combined reasoning"
        }
      }
    ]
  },
...
`keep` contains IDs copied without rewriting. Every `merge` must contain at least two IDs and `base` must be one of them. Omit `replace` when the base text is sufficient. `replace` may contain only string `type`, `description`, or `reasoning` fields, plus boolean `preexisting` for concerns. If merged concerns disagree on `preexisting`, choose the value matching the merged root cause explicitly. Every input ID must appear exactly once across its list's `keep` and `merge` entries. Do not include locations or provenance; Rust derives them from the referenced inputs."#;
...
Your task is to partition each list into unique items and groups that describe the same root cause. Do not reproduce full concern objects.

For an item that is already unique, put its finding ID in `keep`; Rust will copy it unchanged. For overlapping items, emit a `merge` entry containing every matching ID and select the best-written input as `base`. If that base already describes the combined root cause completely, omit `replace` and Rust will use its text unchanged. Use `replace` only when a merged result genuinely needs a new type, description, reasoning, or preexisting value.

Every input ID must occur exactly once in either `keep` or one merge group. Treat concerns and dismissed concerns as separate lists. Do not merge merely related issues: merge only items with the same root cause or failure mode. Rust will union locations and derive all stage, model, and finding-ID provenance; never reproduce or edit those fields."
            }
            9 => {
...
6. A fix owns its own completeness, even over code predating it: if the bug class its commit message claims to close stays reachable, or the correct fix would replace rather than extend its approach, set \"preexisting\": false and name the gap against that claim. Only otherwise say 'This problem wasn't introduced by this patch, but...' and discard low- and medium-severity pre-existing problems, reporting only high and critical.
...
Judge a fix also on whether it achieves its stated purpose. Rate High when the bug class its commit message claims to close stays reachable, or when the correct fix would replace rather than extend its approach: the patch cannot be merged as written and the author must respin, so the consequence to state is a release cycle spent without removing the bug."
            }
            11 => {
...
{}",
            ...
            session
                .require_deduplication_plan(all_concerns.clone(), all_dismissed_concerns.clone())?;
            ...
            let runner = SessionRunner::new(provider.as_ref())
                ...
                // The model gets one correction after Rust reports every plan error.
...
- "preexisting": A boolean value: `true` if this bug/vulnerability already existed in the codebase before these patches were applied, or `false` if the issue was newly introduced by the reviewed patchset. Exception: a fix owns its own completeness, even over code predating it. Set `false` if the bug class its commit message claims to close stays reachable, or if the correct fix would replace rather than extend its approach. Reserve `true` for defects its commit message neither claims to fix nor relies on.
...
#[derive(Clone)]
struct DeduplicationInputItem {
    value: Value,
    order: usize,
}

struct DeduplicationInputs {
    concerns: BTreeMap<String, DeduplicationInputItem>,
    dismissed_concerns: BTreeMap<String, DeduplicationInputItem>,
}

#[derive(Default)]
struct DeduplicationSection {
    keep: Vec<String>,
    merges: Vec<DeduplicationMerge>,
}

struct DeduplicationMerge {
    ids: Vec<String>,
    base: String,
    replacements: serde_json::Map<String, Value>,
}

fn deduplication_input_map(
    items: Vec<Value>,
    label: &str,
) -> Result<BTreeMap<String, DeduplicationInputItem>> {
    ...
    for (order, item) in items.into_iter().enumerate() {
        ...
        if ids.len() != 1 {
            anyhow::bail!("{label} input {order} must have exactly one finding ID");
        }
        let id = ids[0]
            .as_str()
            ...
            .to_string();
        if indexed
            .insert(id.clone(), DeduplicationInputItem { value: item, order })
            .is_some()
        {
            anyhow::bail!("{label} inputs contain duplicate finding ID {id}");
        }
    }
    Ok(indexed)
}

impl DeduplicationInputs {
    fn new(concerns: Vec<Value>, dismissed_concerns: Vec<Value>) -> Result<Self> {
        Ok(Self {
            concerns: deduplication_input_map(concerns, "concern")?,
            dismissed_concerns: deduplication_input_map(dismissed_concerns, "dismissed concern")?,
        })
    }
}

fn string_list(value: Option<&Value>, path: &str, problems: &mut Vec<String>) -> Vec<String> {
...
fn parse_deduplication_replacements(
    value: Option<&Value>,
    path: &str,
    ...
    let Some(object) = value.as_object() else {
        problems.push(format!("{path} must be an object when present"));
        return serde_json::Map::new();
    };
    let mut replacements = serde_json::Map::new();
    for (key, value) in object {
        let valid = match key.as_str() {
            "type" | "description" | "reasoning" => value.is_string(),
            "preexisting" if allow_preexisting => value.is_boolean(),
            _ => {
                problems.push(format!("{path}.{key} is not an allowed replacement field"));
                continue;
            }
        };
        if valid {
            ...
            problems.push(format!("{path}.{key} has the wrong value type"));
        }
    }
    replacements
}

fn parse_deduplication_section(
    value: Option<&Value>,
    path: &str,
    inputs: &BTreeMap<String, DeduplicationInputItem>,
    allow_preexisting: bool,
    problems: &mut Vec<String>,
) -> DeduplicationSection {
    let Some(value) = value else {
        problems.push(format!("{path} is missing"));
        return DeduplicationSection::default();
    };
    let Some(object) = value.as_object() else {
    ...
    for key in object.keys() {
        if !matches!(key.as_str(), "keep" | "merge") {
            problems.push(format!("{path}.{key} is not an allowed field"));
        }
    }

    let keep = string_list(object.get("keep"), &format!("{path}.keep"), problems);
    let mut merges = Vec::new();
    let merge_path = format!("{path}.merge");
    let merge_values = match object.get("merge") {
        None => {
            problems.push(format!("{merge_path} is missing"));
            &[][..]
        }
        Some(value) => match value.as_array() {
            Some(values) => values.as_slice(),
            None => {
                problems.push(format!("{merge_path} must be an array"));
                &[][..]
            }
        },
    };

    for (index, value) in merge_values.iter().enumerate() {
        let item_path = format!("{merge_path}[{index}]");
        let Some(merge) = value.as_object() else {
            problems.push(format!("{item_path} must be an object"));
            continue;
        };
        for key in merge.keys() {
            if !matches!(key.as_str(), "ids" | "base" | "replace") {
                problems.push(format!("{item_path}.{key} is not an allowed field"));
            }
        }
        let ids = string_list(merge.get("ids"), &format!("{item_path}.ids"), problems);
        if ids.len() < 2 {
            problems.push(format!("{item_path}.ids must contain at least two IDs"));
        }
        let base = match merge.get("base").and_then(Value::as_str) {
            Some(base) => base.to_string(),
            None => {
                problems.push(format!("{item_path}.base must be a string"));
                String::new()
            }
        };
        if !base.is_empty() && !ids.contains(&base) {
            problems.push(format!("{item_path}.base must be one of its ids"));
        }
        let replacements = parse_deduplication_replacements(
            merge.get("replace"),
            &format!("{item_path}.replace"),
            allow_preexisting,
            problems,
        );
        if allow_preexisting {
            let preexisting: BTreeSet<bool> = ids
                .iter()
                .filter_map(|id| inputs.get(id))
                .filter_map(|item| item.value.get("preexisting").and_then(Value::as_bool))
                .collect();
            if preexisting.len() > 1 && !replacements.contains_key("preexisting") {
                problems.push(format!(
                    "{item_path}.replace.preexisting is required because the merged inputs disagree"
                ));
            }
        }
        merges.push(DeduplicationMerge {
            ids,
            base,
            replacements,
        });
    ...
    let unknown: Vec<&str> = occurrences
        .keys()
        .copied()
        .filter(|id| !inputs.contains_key(*id))
        .collect();
    if !unknown.is_empty() {
        problems.push(format!(
            "{path} contains unknown IDs: {}",
            unknown.join(", ")
        ));
    }
    ...
    if !duplicates.is_empty() {
        problems.push(format!(
            "{path} contains IDs more than once: {}",
            duplicates.join(", ")
        ));
    }
    let missing: Vec<&str> = inputs
        ...
        .collect();
    if !missing.is_empty() {
        problems.push(format!("{path} omits input IDs: {}", missing.join(", ")));
    }

    DeduplicationSection { keep, merges }
}
...
fn union_locations(
    ...
    for id in ids {
        if let Some(values) = inputs
            .get(id)
            .and_then(|item| item.value.get("locations"))
            .and_then(Value::as_array)
        {
            for value in values {
...
fn materialize_deduplication_group(
    ids: &[String],
    base: &str,
    replacements: &serde_json::Map<String, Value>,
    inputs: &BTreeMap<String, DeduplicationInputItem>,
    ...
    let base = &inputs[base].value;
    let mut output = serde_json::Map::new();
    for key in ["type", "description", "reasoning"] {
        if let Some(value) = base.get(key) {
            output.insert(key.to_string(), value.clone());
        }
    }
    if include_provenance && let Some(value) = base.get("preexisting") {
        output.insert("preexisting".to_string(), value.clone());
    }
    for (key, value) in replacements {
        output.insert(key.clone(), value.clone());
    }
    output.insert("locations".to_string(), json!(union_locations(ids, inputs)));

    if include_provenance {
        let source_stages: BTreeSet<u64> = ids
            .iter()
            .filter_map(|id| inputs.get(id))
            .flat_map(|item| {
                item.value
            ...
            .collect();
        let source_models: BTreeSet<&str> = ids
            .iter()
            .filter_map(|id| inputs.get(id))
            .flat_map(|item| {
                item.value
            ...
            .collect();
        output.insert("source_stages".to_string(), json!(source_stages));
        output.insert("source_models".to_string(), json!(source_models));
        output.insert(
            "finding_ids".to_string(),
            json!(ids.iter().collect::<BTreeSet<_>>()),
        );
    }
    Value::Object(output)
}

fn materialize_deduplication_section(
    section: DeduplicationSection,
    inputs: &BTreeMap<String, DeduplicationInputItem>,
    include_provenance: bool,
) -> Vec<Value> {
    let mut groups: Vec<(usize, Vec<String>, String, serde_json::Map<String, Value>)> = section
        .keep
        .into_iter()
        .map(|id| {
            let order = inputs[&id].order;
            (order, vec![id.clone()], id, serde_json::Map::new())
        })
        .collect();
    groups.extend(section.merges.into_iter().map(|merge| {
        let order = merge
            .ids
            .iter()
            .filter_map(|id| inputs.get(id).map(|item| item.order))
            .min()
            .unwrap_or(usize::MAX);
        (order, merge.ids, merge.base, merge.replacements)
    }));
    groups.sort_by_key(|group| group.0);
    groups
        .into_iter()
        .map(|(_, ids, base, replacements)| {
            materialize_deduplication_group(&ids, &base, &replacements, inputs, include_provenance)
        })
        .collect()
}

fn validate_and_materialize_deduplication_plan(
    plan: &Value,
    inputs: &DeduplicationInputs,
) -> std::result::Result<Value, Vec<String>> {
    let mut problems = Vec::new();
    let Some(object) = plan.as_object() else {
        return Err(vec!["top-level response must be an object".to_string()]);
    };
    for key in object.keys() {
        if !matches!(key.as_str(), "concerns" | "dismissed_concerns") {
            problems.push(format!("top-level field {key} is not allowed"));
        }
    }
    let concerns = parse_deduplication_section(
        object.get("concerns"),
        "concerns",
        &inputs.concerns,
        true,
    ...
    let dismissed_concerns = parse_deduplication_section(
        object.get("dismissed_concerns"),
        "dismissed_concerns",
        &inputs.dismissed_concerns,
        false,
        &mut problems,
    );
    ...
    Ok(json!({
        "concerns": materialize_deduplication_section(concerns, &inputs.concerns, true),
        "dismissed_concerns": materialize_deduplication_section(
            dismissed_concerns,
            &inputs.dismissed_concerns,
            false,
        ),
...
fn format_deduplication_problems(problems: &[String]) -> String {
    let mut message = String::from("Rust found these problems in the compact plan:");
...
impl ReviewStageSession {
    ...
    fn require_deduplication_plan(
        &mut self,
        concerns: Vec<Value>,
        dismissed_concerns: Vec<Value>,
    ) -> Result<()> {
        self.required_deduplication_inputs =
            Some(DeduplicationInputs::new(concerns, dismissed_concerns)?);
        Ok(())
    }
...
impl Drop for ReviewStageSession {
    fn drop(&mut self) {
        self.finish_response_health(crate::json_health::JsonDecodeOutcome::Fatal);
    }
}
...
impl LlmSession for ReviewStageSession {
    ...
    fn validate(&mut self, response: &AiResponse) -> Result<Self::Output, ValidationError> {
        ...
        let result = (|| {
            let mut output = self.stage.validate(response)?;
            if let Some(inputs) = &self.required_deduplication_inputs {
                match validate_and_materialize_deduplication_plan(&output, inputs) {
        ...
        if result.is_ok() {
            self.finish_response_health(crate::json_health::JsonDecodeOutcome::RecoveredOnRetry);
        }
        result
    }
...
mod tests {
    ...
    #[test]
    fn compact_deduplication_plan_materializes_full_concerns() {
        let inputs = DeduplicationInputs::new(
            vec![
                deduplication_test_item("a", 1, "main", "keep this", false, "a.c"),
                deduplication_test_item("b", 3, "main", "use this base", false, "b.c"),
                deduplication_test_item("c", 4, "variant", "duplicate", false, "c.c"),
            ],
            vec![
                deduplication_test_item("d", 5, "main", "dismissed base", false, "d.c"),
                deduplication_test_item("e", 5, "variant", "dismissed duplicate", false, "e.c"),
            ],
        )
        .unwrap();
        let plan = json!({
            ...
            "dismissed_concerns": {
                ...
                "merge": [{
                    "ids": ["d", "e"],
                    "base": "d",
                    "replace": {"reasoning": "combined dismissal evidence"}
                }]
            }
        });

        let output = validate_and_materialize_deduplication_plan(&plan, &inputs).unwrap();
        let concerns = output["concerns"].as_array().unwrap();
        assert_eq!(concerns.len(), 2);
        assert_eq!(concerns[0]["description"], "keep this");
        assert_eq!(concerns[0]["source_stages"], json!([1]));
        assert_eq!(concerns[1]["description"], "use this base");
        assert_eq!(concerns[1]["reasoning"], "reasoning for use this base");
        assert_eq!(concerns[1]["source_stages"], json!([3, 4]));
        assert_eq!(concerns[1]["source_models"], json!(["main", "variant"]));
        assert_eq!(concerns[1]["finding_ids"], json!(["b", "c"]));
        assert_eq!(
            concerns[1]["locations"],
            json!([{"file": "b.c"}, {"file": "c.c"}])
        );

        let dismissed = &output["dismissed_concerns"][0];
        assert_eq!(dismissed["description"], "dismissed base");
        assert_eq!(dismissed["reasoning"], "combined dismissal evidence");
        assert_eq!(
            dismissed["locations"],
            json!([{"file": "d.c"}, {"file": "e.c"}])
        );
        assert!(dismissed.get("finding_ids").is_none());
    }

    #[test]
    fn compact_deduplication_requires_resolution_of_preexisting_conflicts() {
        let inputs = DeduplicationInputs::new(
            vec![
                deduplication_test_item("a", 1, "main", "base", false, "a.c"),
                deduplication_test_item("b", 1, "variant", "duplicate", true, "b.c"),
            ],
            Vec::new(),
        )
        .unwrap();
        let unresolved = json!({
            "concerns": {
                "keep": [],
                "merge": [{"ids": ["a", "b"], "base": "a"}]
            },
            "dismissed_concerns": {"keep": [], "merge": []}
        });
        let problems =
            validate_and_materialize_deduplication_plan(&unresolved, &inputs).unwrap_err();
        assert!(problems.iter().any(|problem| {
            problem.contains("replace.preexisting is required because the merged inputs disagree")
        }));
        ...
        let resolved = json!({
            "concerns": {
                ...
                "merge": [{
                    "ids": ["a", "b"],
                    "base": "a",
                    "replace": {"preexisting": true}
                }]
            },
            "dismissed_concerns": {"keep": [], "merge": []}
        });
        let output = validate_and_materialize_deduplication_plan(&resolved, &inputs).unwrap();
        assert_eq!(output["concerns"][0]["preexisting"], true);
    }

    #[test]
    fn stage_eight_session_returns_materialized_concerns() {
        let temp = tempfile::tempdir().unwrap();
        let mut session = ReviewStageSession::new(
            create_stage(8),
            String::new(),
        ...
        session
            ...
            .unwrap();
        let response = stage_test_response(
            r#"{
                "concerns": {"keep": ["a"], "merge": []},
                "dismissed_concerns": {"keep": [], "merge": []}
            }"#,
        );
        ...
        assert!(output["concerns"].is_array());
        assert_eq!(output["concerns"][0]["description"], "unchanged");
        assert_eq!(output["concerns"][0]["source_stages"], json!([1]));
        assert!(output["dismissed_concerns"].is_array());
    }

    #[test]
    fn recovered_stage_eight_assembly_error_is_recorded() {
        ...
        assert_eq!(events[0].source, STAGE8_RESPONSE_ASSEMBLY_SOURCE);
        assert_eq!(
            events[0].outcome,
            crate::json_health::JsonDecodeOutcome::RecoveredOnRetry
        );
        assert!(events[0].detail.contains("unknown IDs: unknown"));
    }

    #[test]
    fn compact_deduplication_validation_reports_all_detected_problems() {
        let inputs = DeduplicationInputs::new(
            vec![
                deduplication_test_item("a", 1, "main", "a", false, "a.c"),
                deduplication_test_item("b", 2, "main", "b", false, "b.c"),
                deduplication_test_item("c", 3, "main", "c", true, "c.c"),
            ],
            Vec::new(),
        )
        .unwrap();
        let plan = json!({
            ...
            "concerns": {
                ...
                "merge": [{
                    "ids": ["b"],
                    "base": "missing",
                    "replace": {"locations": [], "preexisting": "no"}
                }],
                "extra": []
            },
            "dismissed_concerns": {"keep": [], "merge": []}
        ...
        let problems = validate_and_materialize_deduplication_plan(&plan, &inputs).unwrap_err();
        let message = format_deduplication_problems(&problems);
        for expected in [
            "top-level field unexpected is not allowed",
            "concerns.extra is not an allowed field",
            "concerns.merge[0].ids must contain at least two IDs",
            "concerns.merge[0].base must be one of its ids",
            "concerns.merge[0].replace.locations is not an allowed replacement field",
            "concerns.merge[0].replace.preexisting has the wrong value type",
            "concerns contains unknown IDs: unknown",
            "concerns contains IDs more than once: a",
            "concerns omits input IDs: c",
        ] {
            assert!(
    ...
    struct InvalidDeduplicationProvider {
        calls: std::sync::atomic::AtomicUsize,
        requests: std::sync::Mutex<Vec<String>>,
    }

    #[async_trait::async_trait]
    impl crate::ai::AiProvider for InvalidDeduplicationProvider {
        ...
        fn get_capabilities(&self) -> crate::ai::ProviderCapabilities {
            crate::ai::ProviderCapabilities {
                model_name: "invalid-deduplication".to_string(),
    ...
    #[tokio::test]
    async fn stage_eight_gets_one_retry_with_every_validation_problem() {
        let _guard = crate::json_health::TEST_GUARD.lock().await;
        crate::json_health::drain();
        let temp = tempfile::tempdir().unwrap();
        let provider = InvalidDeduplicationProvider {
        ...
        let error = match result {
            Ok(_) => panic!("invalid Stage 8 plans unexpectedly passed validation"),
            Err(error) => error,
        };

        assert!(error.to_string().contains("after 2 validation attempts"));
        assert_eq!(provider.calls.load(std::sync::atomic::Ordering::SeqCst), 2);
        let requests = provider.requests.lock().unwrap();
        assert_eq!(requests.len(), 2);
        assert!(requests[1].contains("Rust found these problems"));
        assert!(requests[1].contains("concerns contains unknown IDs: unknown"));
        assert!(requests[1].contains("concerns omits input IDs: a, b"));
        drop(requests);
        drop(session);
