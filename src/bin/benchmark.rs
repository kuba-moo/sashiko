...
use reqwest::Client;
use sashiko::ai::{
    AiProvider, AiResponseFormat, LlmSession, SessionRunner, ValidationError, create_provider,
};
...
async fn main() -> Result<()> {
    ...
    if let Some(corpus_path) = &args.corpus {
        run_corpus_benchmark(corpus_path, port, &settings, db, args.analyze_only).await
    } else if let Some(file_path) = &args.file {
        run_json_benchmark(file_path, port, &settings, db, args.repo, args.analyze_only).await
    } else {
...
async fn run_corpus_benchmark(
    ...
    if !analyze_only {
        ...
        for case in &cases {
            info!(
                "Submitting case: {} (test_message_id: {})",
                case.name, case.test_message_id
            );
    ...
    info!(
        ...
        if total_annotations > 0 {
            total_detected as f64 / total_annotations as f64 * 100.0
        } else {
            0.0
        }
    );
    info!(
        ...
        total_partially,
        if total_annotations > 0 {
            total_partially as f64 / total_annotations as f64 * 100.0
        } else {
            0.0
        }
    );
    info!(
        ...
        total_missed,
        if total_annotations > 0 {
            total_missed as f64 / total_annotations as f64 * 100.0
        } else {
            0.0
        }
    );
    ...
    info!(
        ...
        if total_findings_count > 0 {
            total_false_positives as f64 / total_findings_count as f64 * 100.0
        } else {
            0.0
        }
    );

    if let Some(avg_tokens_in) = total_tokens_in.checked_div(valid_metric_count) {
        info!("--- Performance Metrics (averages per case) ---");
        info!("Avg Tokens In:  {}", avg_tokens_in);
...
impl LlmSession for BenchmarkEvalSession {
    ...
    fn validate(
        ...
        Ok(response.content.as_deref().unwrap_or("").to_string())
    }
...
fn inject_message_ids(case_name: &str, mbox_raw: &str) -> Result<(String, Vec<String>)> {
    ...
    for (i, email_bytes) in emails.iter().enumerate() {
        ...
        if i > 0 {
            headers_to_inject.push_str(&format!("In-Reply-To: <{}>\n", message_ids[0]));
        }
...
struct CorpusJudgeSession {
    prompt: String,
    schema: serde_json::Value,
}

#[async_trait]
impl LlmSession for CorpusJudgeSession {
    type Output = JudgeResponse;
    ...
    fn validate(
        ...
        serde_json::from_str(response.content.as_deref().unwrap_or(""))
            .map_err(|error| ValidationError::FormatViolation(error.to_string()))
    }
}
...
async fn process_corpus_case(
    ...
    let mut session = CorpusJudgeSession { prompt, schema };
    let judge_result = match SessionRunner::new(client.as_ref()).run(&mut session).await {
        Ok(result) => result.output,
        Err(e) => {
            warn!(
                "Failed to evaluate judge response for case {}: {}",
                case.name, e
            );
            JudgeResponse {
                annotation_results: case
                    ...
                    .map(|a| AnnotationResult {
                        ...
                        matched_finding: None,
                        explanation: format!("Judge response error: {}", e),
                    })
                    .collect(),
                false_positives: vec![],
            }
...
mod tests {
    ...
    fn test_load_corpus_case() {
        ...
        assert!(
            case.mbox_content
                .contains("Message-ID: <case-001@sashiko-benchmark>")
        );
