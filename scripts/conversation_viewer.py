...
STAGE_NAMES = {
    "s0": "Phase 0: Guide Pre-screening",
    "sp": "Planning",
    "s1": "Stage 1: Commit Goal",
    "s2": "Stage 2: Implementation",
    "s3": "Stage 3: Execution Flow",
    "s4": "Stage 4: Resource Mgmt",
    "s5": "Stage 5: Locking",
    "s6": "Stage 6: Security",
    "s7": "Stage 7: Hardware",
    "s8": "Stage 8: Deduplication",
    "s9": "Stage 9: Conflict Resolution",
    "s10": "Stage 10: Verification",
    "s11": "Stage 11: Report",
}


def stage_label(stage: str) -> str:
    """Display name for a dump label, tolerating stages added since this map."""
    if stage in STAGE_NAMES:
        return STAGE_NAMES[stage]
    if stage.startswith("s") and stage[1:].isdigit():
        return f"Stage {int(stage[1:])}"
    return stage
...
class ConversationViewer(App):
    ...
    @staticmethod
    def _stage_sort_key(s: str) -> tuple[int, int, str]:
        if s == "s0":
            return (0, 0, "")
        if s == "sp":
            return (1, 0, "")
        # Numbered stages sort numerically so s10/s11 land after s9, not after s1.
