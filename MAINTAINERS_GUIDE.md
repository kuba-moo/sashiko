...
*   **`embargo_hours`:** The number of hours to wait before publishing a review with findings. Reviews with no findings are released immediately after the complete patchset review succeeds. When a patch is sent to multiple mailing lists, the shortest explicitly configured embargo period among the matched subsystems will be used. If no matched subsystems explicitly configure this value, it falls back to the default policy. An operator can publish a held review ahead of that window with [`sashiko-cli lift-embargo`](docs/sashiko-cli.md#lift-embargo). Where a release schedule is configured it can move the window either way, but never past the four-day maximum measured from when Sashiko recorded the series ([`max_hold_hours`](docs/configuration.md#embargo)), so a value above that is only an upper bound on paper.
